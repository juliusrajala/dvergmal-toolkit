...
interface Props {
  gameId: number;
  playerId: number;
  initialRolls: Array<DieRoll>;
  initialPrompts: PromptWithRelatedRolls[];
}

const fetchDieRolls = async (
  gameId: number,
): Promise<{ dieRolls: Array<DieRoll> }> => {
...
const mapEventsOnTray = (
  ...
  return events
    ...
    .map((event) =>
      "rollTotal" in event ? (
        <RollItem
          key={`roll-${event.id}-${event.createdAt}`}
          roll={event}
          ownId={playerId}
        />
      ) : (
        <PromptItem
          key={`prompt-${event.id}-${event.createdAt}`}
          prompt={event}
        />
      ),
    );
...
const DiceTray = ({
  ...
  const [rolls, setRolls] = useState<Array<DieRoll>>(initialRolls); // Fetch rolls from the database
  ...
  useEffect(() => {
    const source = new EventSource("/api/game/dice/sse");

    source.onmessage = (event) => {
      console.log(event, new Date().toISOString());
    };
    return () => {
      console.log("Dismounting and closing source");
      return source.close();
    };
  }, []);
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex-row justify-between flex">
        <h2 className="font-semibold text-lg">Rolls & Reactions</h2>
        <div className="flex gap-5">
          <button
            className="btn btn-secondary btn-xs"
            onClick={async () => {
              await fetch("/api/game/dice/sse", {
                method: "POST",
                headers: {
                  "Content-Type": "application/json",
                },
          ...
          <label className="text-sm flex gap-2">
            <input
              ...
              onChange={() => setOnlyShowMe(!onlyShowMe)}
            />
            Only me
          </label>
          ...
          <label className="text-sm flex gap-2">
            <input
              type="checkbox"
              className="toggle toggle-sm toggle-primary"
              onChange={() => setHidePrompts(!hidePrompts)}
            />
            Hide prompts
          </label>
          <label className="text-sm flex gap-2">
            <input
              type="checkbox"
              className="toggle toggle-sm toggle-primary"
              onChange={() => setHideRolls(!hideRolls)}
            />
            Hide rolls
          </label>
        </div>
