...
export default async function seed() {
  ...
  if (!invitationCode) {
    throw new Error(
      "INVITATION_CODE environment variable is not set. Set up your .env file.",
    );
  }

  if (!seed_data || users.length === 0) {
    throw new Error(
      "No seed data found. Please create a seed_data.json file based on seed_data.example.json",
    );
  }
  ...
  for await (const game of games) {
    ...
    if (!ownerId) {
      console.warn(
        `Owner with email ${game.ownerEmail} not found, skipping game ${game.name}`,
      );
      continue;
    }
    ...
    if (game.joinAll) {
      ...
      for await (const [index, player] of players.entries()) {
        ...
        const characterName =
          characterNames[index] || `Adventurer ${index + 1}`;
        await joinGameByNameAndSecret(
          player.id,
          game.name,
          game.secret,
          characterName,
